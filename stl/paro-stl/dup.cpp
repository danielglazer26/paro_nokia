...
vector<T> unique(vector<T> const &first, vector<T> const &second) {
    vector<T> allNumberVector = first;
    allNumberVector.insert(allNumberVector.end(), second.begin(), second.end());

    unordered_set<T> uniqueNumbers(allNumberVector.begin(), allNumberVector.end());
    allNumberVector.assign(uniqueNumbers.begin(), uniqueNumbers.end());

    return allNumberVector;
}
